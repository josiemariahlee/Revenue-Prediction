...
train <- read.csv("train.csv",header=TRUE,dec = ".",sep=",",stringsAsFactors=FALSE,fileEncoding="UTF8")
test <- read.csv("test.csv",header=TRUE,dec = ".",sep=",",stringsAsFactors=FALSE,fileEncoding="UTF8")
...
# nah, nevermind partition for validation, we will use Leaderboard for that, and caret will do cross-validation
datatrain <- train
datatest  <- test
